...
const RetirementGauge = ({ progress }) => {
    // Determine the category and color based on the progress value
    let category, color;
    if (progress < 50) {
      category = 'Falling Short';
      color = '#EAEAEA';
    } else if (progress < 80) {
    ...
    return (
      <div>
        <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 0 100 50">
          {/* Define the first (background) semi-circle */}
          <path d="M0 50 A50 50 0 1 1 100 50" fill={color} />
  
          {/* Define the second semi-circle representing progress */}
          <path
            d={`M0 50 A50 50 0 ${progress > 50 ? 1 : 0} 1 ${Math.cos((progress / 100) * Math.PI)} ${
              50 - Math.sin((progress / 100) * Math.PI) * 50
            } L50 50 Z`}
            fill={color}
          />
  
          {/* Define the gauge needle */}
          <line
            x1="50"
            y1="50"
            x2="50"
...
export default function Dashboard() {
  ...
  <Title variant="h4" sx={{color: "#042A57"}}>
      ...
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column', height: "100%" }}>
        {/* Render the RetirementGauge component */}
        <RetirementGauge progress={retirementProgress} />
      </Paper>
    </Grid>
              {/* Recent Orders */}
              <Grid item xs={12}>
